...
while channel2:
    ...
    for x in itertools.combinations(channel2, i):
        ...
        if total_occurence / transaction <= min_support and total_occurence / transaction > 0:
...
del (rules[cnt])
print(rules)
rules = sorted(rules, key=lambda q: (q[3], q[5], q[6], (1/q[4]) ), reverse=True) # Ranked on Gravity

n_rules = [[]]
for i, e in enumerate(rules):
    # Things as it appears in the EXCEL file
...
with open("result_5k(CC1).csv", "w") as ff:
